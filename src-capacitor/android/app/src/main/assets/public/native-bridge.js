...
(function (win) {
  ...
  const useFallbackLogging = Object.keys(win.console).length === 0;
  if (useFallbackLogging) {
    win.console.warn('Advance console logging disabled.');
  }

  // patch window.console on iOS and store original console fns
  const orgConsole = capacitor.isIOS ? {} : win.console;
    ...
    Object.keys(win.console).forEach((level) => {
      if (typeof win.console[level] === 'function') {
        ...
        win.console[level] = function capacitorConsole() {
          let msgs = Array.prototype.slice.call(arguments);
  ...
  function addLegacyHandlers(win) {
    ...
    document.addEventListener = function () {
      ...
      if (name === 'deviceready') {
        ...
        // back button action
        Capacitor.Plugins.App && Capacitor.Plugins.App.addListener('backButton', () => {});
      }
  ...
  capacitor.addListener = function (pluginId, eventName, callback) {
    const callbackId = capacitor.nativeCallback(pluginId, 'addListener', {
      eventName,
    }, callback);
    return {
      remove() {
  ...
  win.Ionic.WebView.getServerBasePath = function (callback) {
    Capacitor.Plugins.WebView.getServerBasePath().then((result) => {
